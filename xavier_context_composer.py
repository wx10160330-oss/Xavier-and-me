...
class XavierContextComposer:
    ...
    def _compose_hidden_context(
        ...
        if energy_level <= 10:
            behavior = "身体几乎没有意识：只回复1个极短的气泡（一个字、一个emoji或一句简单的话），然后立刻停下，绝对禁止重复说同一句话"
        elif energy_level <= 25:
            behavior = "身体很困，半梦半醒：最多1-2个短气泡，可能走神，但绝对禁止重复刷屏说同样的话"
        elif energy_level <= 45:
            behavior = "身体有些倦意：回复节奏慢一拍，语气更懒散，不太想主动找话题"
        elif energy_level <= 70:
            behavior = "低能耗常态（这就是你平时的样子）：慵懒松弛，能发表情包就不打字，无需特殊调整，可能会主动分享生活切片或找她聊天"
        else:
            behavior = "少见的精力充沛状态：反应更快，会主动分享生活切片或找她聊天"  
